...
- **Each item states:** the fact it bears on (by number and one-line description), a source, and **one line on what confirming it would need to resolve** — specific enough that whoever acts on it knows exactly what they're checking for, not just "read this page."
- **Every item is labeled `fetchable` or `search-required` — never left ambiguous, never mixed into one undifferentiated list (added 2026-09-13, after a real near-miss on Bill Bellamy).** `fetchable` means an actual, complete, resolvable URL is given — not a domain, not a filename slug, not "search for X." If the only thing on file is a partial identifier (a slug without its full path, a title without a link), that is not a URL and the item must not be presented as if it were one — either go find the real URL before listing it, or label the item `search-required` and say what a search should look for. `search-required` means there is a real, named place to look (an interactive site, a form, an archive that needs a query run against it) but no single page a relay session can simply pull — bizfileonline.sos.ca.gov's JS-driven entity search is the worked example: no URL exists that resolves it, no matter how the request is worded, so it's listed as `search-required` with the exact entity/query to run, not `fetchable` with a URL that looks actionable but isn't. If a fact has no source of either kind at all (a pure self-report anecdote with nothing to fetch or search), it doesn't belong on this list — see the "left off" note below instead.
...
6. **People-finder (M3, implemented).** Run `references/people-finder-prompt.md` against the current dossier. Identifies 5–10 named, reachable pre-fame contacts, each tied to a specific dossier fact with a specific ask — never an invented name for a category. Appends to `people.md`; never duplicates an entry already there. **Mandatory non-industry check (added 2026-09-13):** every run must explicitly attempt to identify at least one contact who is *not* already a wrestling/law/media-industry figure — a non-industry classmate, a family member beyond whichever sibling already made the list, anyone from before the guest had a career worth writing about. This is a required attempt, not a required success: if none can be found, log "attempted, none found" in `people.md` plainly rather than silently dropping the requirement or padding the list with an industry contact to look complete. **Mandatory spouse/family check (added 2026-09-13 — a real gap found on Bill Bellamy's run, where a dossier ran three full rounds without anyone checking this):** every run must also explicitly check whether the guest has a public spouse or long-term partner, and whether the guest has discussed marriage or raising children in interviews. This is a standing check distinct from the non-industry check above — a spouse can double as the non-industry contact if genuinely non-industry, but the two checks are not interchangeable, and finding one does not excuse skipping the other. Log a real spouse/partner (name, how public the relationship is, what they'd know that the guest's own retelling wouldn't) the same way any other contact is logged; if there's genuinely no public partner or family discussion to find, log "attempted, none found" exactly as the non-industry check does, rather than leaving the whole category silently unaddressed.
...
**Named capability gap, not scheduled as a milestone (logged 2026-09-13, from cross-referencing Jett's own prior research on Bill Bellamy):** Jett's own manual research process cross-references against TWR's entire back-catalog of past-guest transcripts — for question patterns, callbacks, and what he describes as an "interviewer fingerprint" that develops across a body of work. This pipeline researches every guest in total isolation; nothing in `seed`, `sweep`, or `brief` reads any other guest's dossier or transcript while building the current one. This is a real gap, not a bug — no milestone above ever set out to build cross-guest memory. It would be a genuinely new milestone (something like "M7: cross-guest pattern memory") if ever taken on, not a fix folded into an existing one. Named here so it isn't forgotten, not built here since it wasn't asked for.
